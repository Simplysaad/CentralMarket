...
Search.find({}, { searchTerm: 1 }).then(
    data => (locals.searchSuggestions = data)
);
...
router.get("/dashboard", async (req, res) => {
    try {
        ...
        const completedOrders = currentUserOrders.filter(
        ...
        const mostSearched = await Search.find(
            {},
            { searchResults: 1 }
        ).populate("searchResults.productId");
...
router.post(
    ...
    async (req, res) => {
        try {
            let images = req.files.productImage;
            ...
            let productImage = productGallery[0];

            const { productTags, name, basePrice, discount } = req.body;
...
//         //let reqFile = req.files[0].productImage;
//         let file = req.files.productImage;

//         // let productGallery = [];
//         // for (let file of req.files) {
//         let imageData = await uploadToImgur(file);
//         //   productGallery.push(imageData.link);
//         //  }

//         //let productImage = productGallery[0];
//         console.log("imageData", imageData);
//         let productImage = imageData.link;
...
router.get("/orders/:id", async (req, res) => {
    try {
        ...
        if (!currentUserId || !currentUser) return res.redirect("auth/login");
