...
addProductsForm.addEventListener("submit", e => {
  ...
  actualFormData.append("productImage", productImage.value);
  actualFormData.append("product", formDataObject);

  console.log(JSON.stringify(actualFormData));

  fetch("/vendor/products/add", {
    method: "post",
    headers: {
    ...
    .then(data => {
      ...
      if (data.success) {
        window.location.href = `/preview/${data.savedProduct._id}`
      }
    })
