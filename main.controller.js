...
const getIndexRoute = async (req, res) => {
  try {
    const popularProducts = await Product.find()//= await getPopularProducts();
    const featuredProducts = await Product.find({
      isFeatured: true
    }).sort({
      updatedAt: -1
    });
    const recentSearches = await Search.find() //getRecentSearches();
    ...
    .limit(11);
    const todayDeals = await Product.find({
      discount: {
        $exists: true
      }
    })
...
const postProductReviews = async (req, res) => {
  try {
    ...
    let newNotification = {
      ...
      ? reviewText.substring(0, 20) + "...": reviewText,
      type: "review",
    ...
    .status(500)
    .json({
      message: "Error saving review", error: err.message
    });
  }
}
...
const getStore = async (req, res) => {
  try {
    ...
    vendor.products = await Product.find({
      vendorId: vendor._id
    });
...
const postStore = async (req, res) => {
  try {
    ...
    let newNotification = {
      ...
      body:
      messageText > 40
      ? messageText.substring(0, 40) + "...": messageText,
      type: "message",
...
const getPreview = async (req, res) => {
  try {
    ...
    ? 0: overallRating / currentProductReviews.length;
    console.log({
...
const postCart = async (req, res) => {
  try {
    ...
    const itemIndex = cartItems.findIndex(
      item =>
      item.productId === singleItem.productId &&
    ...
    req.session.save(err => {
      if (err) {
        throw new Error("Error encountered while saving session");
      }
    });
    return res.redirect("/cart");
  } catch (err) {
    console.error("Error in add-cart route:",
      err);
    res.status(502).redirect("/502");
  }
}
const deleteCart = async (req, res) => {
  try {
    ...
    if (!currentProduct)
      ...
      success: false,
      message: `product with ID: ${req.params.id} not found in cart`
    })
    throw new Error(
    ...
    console.error("Error in remove-item route:",
      err);
    res.status(502).redirect("/502");
  }
}
